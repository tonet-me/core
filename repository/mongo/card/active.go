...
func (d DB) ActiveCard(ctx context.Context, cardID string) (bool, error) {
	const op = richerror.OP("cardmongo.ActiveUser")
	...
	filter := bson.D{{"_id", id}, {"status", entity.CardStatusDeActive}}

	update := bson.D{{"$set", bson.D{{"status", entity.CardStatusActive}}}}
