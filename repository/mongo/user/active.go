...
func (d DB) ActiveUser(ctx context.Context, userID string) (bool, error) {
	const op = richerror.OP("usermongo.ActiveUser")
	...
	filter := bson.D{{"_id", id}, {"status", entity.UserStatusDeActive}}

	update := bson.D{{"$set", bson.D{{"status", entity.UserStatusActive}}}}
